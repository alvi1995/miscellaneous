...
class bandMatrix : public Matrix<T> 
{
	...
	public:
		bandMatrix(double *element, Vector<T> vec, int column, int diag_size); // in compact form
		void compact(bool b);
		~bandMatrix();
};
...
template<class T>
bandMatrix::bandMatrix(double *element, Vector<T> vec, int Or_column, int diag_size)
{
	int i, j;
...
void bandMatrix::compact(bool b)
{
